function Bird(img) {
  ...
  this.draw = (ctx) => {
    ...
    if(this.estado == estadoJogo.inicio){
      ctx.drawImage(tela, 0, 0, tela.width, tela.height, cnv.width / 2 - 100, cnv.height / 2 - 50, 200, 100)
      ctx.fillStyle = '#00f'
      ctx.font = "25px Arial"
      ctx.fillText('Pressione (V) para Blue Bird.', cnv.width / 2 - 155, cnv.height / 2 + 75)
      ctx.fillStyle = '#f00'
      ctx.fillText('Pressione (C) para Red Bird.', cnv.width / 2 - 156, cnv.height / 2 + 110)
    }
    /* TELA DE PERDEU */
    if(this.estado == estadoJogo.perdeu){
      ...
      ctx.fillStyle = '#f0f'
      ctx.font = "25px Arial"
      ctx.fillText('Precione (R) para recomeçar.', cnv.width / 2 - 160, cnv.height / 2 + 75)
    }
  }
  ...
  this.colision = (cano) => {
    if((this.x + 30 > cano.x && this.x < cano.x + 35) && (this.y < cano.y + cano.altura)){
      cano.cor = 'red'
    ...
    else if((this.x + 30 > cano.x && this.x < cano.x + 35) && (this.y + 30 > cano.yB)){
      cano.cor = 'red'
      this.estado = estadoJogo.perdeu
      this.forçaPulo = 0
      cano.speed = 0
  ...
  this.reset = () => {
    ...
    this.x = 30
    this.y = cnv.height / 2 - 32
    cano1.x = cnv.width + 100
    cano2.x = cnv.width + 300
    cano1.speed = 3
    cano2.speed = 3
    this.forçaPulo = -6
  }
